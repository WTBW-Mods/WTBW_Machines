...
public class DehydratingCategory extends AbstractRecipeCategory<DehydratingRecipe>
{
  ...
  @Override
  public void draw(DehydratingRecipe recipe, MatrixStack stack, double mouseX, double mouseY)
  {
    int x = halfX - 5;
    int y = halfY - 5;
    DehydratorScreen.PROGRESS_BACKGROUND.render(stack, x, y);
  ...
  @Override
  public List<ITextComponent> getTooltipStrings(DehydratingRecipe recipe, double mouseX, double mouseY)
  {
    energyBar.storage.setEnergy(recipe.powerCost);
    energyBar.update();
